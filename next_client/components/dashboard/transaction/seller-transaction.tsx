...
import { transferSellerNFT } from "@/utils/transaction-property";
...
export default function SellerTransaction() {
    ...
    const { getSellerTransactions, isloading, transactions, updateSellStatus } =
        useSellTransactionStore();
    ...
    const handleAccept = async () => {
        ...
        await transferSellerNFT(selectedTransaction?.property.tokenID)
            .then(async (txHash) => {
                console.log("NFT Transferred. Tx Hash:", txHash);
                toast({
                    title: "Success",
                    description: "NFT Transferred to escrow account",
                });

                if (selectedTransaction?.id && selectedTransaction?.sellerId) {
                    const data = updateSellStatus(
                    ...
                    if ((await data).success) {
                        toast({
                            title: "Success",
                            description: "Metaserver updated",
                        });
                        router.push("/dashboard");
                    } else {
                        toast({
                            title: "Error",
                            description: "Failed to update Metaserver",
                            variant: "destructive",
                        });
                        router.push("/dashboard");
                    }
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <p className="text-lg font-medium">
                                Asking Price: $
                                {parseInt(
                                    selectedTransaction.property.price
                                ).toLocaleString()}
                            </p>
                        </div>
                        <div className="flex items-center space-x-2">
                        ...
                        <div className="mt-4">
                            <Badge
                                variant={
                                    ...
                                    parseInt(selectedTransaction.property.price)
                                        ? "default"
                                        : "destructive"
                                }
                                className="text-sm"
